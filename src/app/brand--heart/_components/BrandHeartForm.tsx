...
export function BrandHeartForm({ 
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (name.includes('_')) {
            ...
            if ((lang === 'primary' || lang === 'secondary') && field && field in brandHeart) {
                setBrandHeart((prev: any) => ({
                    ...prev,
                    [field]: { ...(prev[field] || { primary: '', secondary: '' }), [lang]: value },
                }));
                return;
            }
        }
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        Object.keys(brandHeart).forEach(key => {
            if (typeof brandHeart[key] === 'object' && brandHeart[key] !== null) {
                if ('primary' in brandHeart[key] && brandHeart[key].primary) {
                    formData.append(`${key}_primary`, brandHeart[key].primary);
                }
                 if ('secondary' in brandHeart[key] && brandHeart[key].secondary) {
