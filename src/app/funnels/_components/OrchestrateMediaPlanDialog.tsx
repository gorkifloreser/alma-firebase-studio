...
type ViewState = 'list' | 'generate' | 'edit';
...
export function OrchestrateMediaPlanDialog({
    ...
    const renderEditView = () => (
                                        ...
                                        <div className={cn("space-y-4", isSelectionMode && "pl-8")}>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Input type="time" value={timeValue} onChange={(e) => { const [hours, minutes] = e.target.value.split(':').map(Number); const newDate = postDate ? setHours(setMinutes(postDate, minutes), hours) : null; handleItemChange(item.id, 'suggested_post_at', newDate?.toISOString() || ''); }} className="w-[120px]" />
                                                </div>
                                            </div>
        ...
        ) : (isGenerating ? <div className="space-y-4 p-4"><Skeleton className="h-24 w-full" /><Skeleton className="h-24 w-full" /><Skeleton className="h-24 w-full" /></div> : <p>Something went wrong.</p>)
    );
